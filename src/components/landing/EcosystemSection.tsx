  ...
  return (
    <section className="relative w-full bg-[#f6f8fb] py-16 sm:py-24 md:py-32 px-4 sm:px-6 lg:px-8 font-sans overflow-hidden">
      <div className="max-w-6xl mx-auto">
        ...
        {/* ─── Large Island Card Container ─────────────────────────────────── */}
        <motion.div
          initial={{ opacity: 0, y: 32 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, margin: "-60px" }}
          transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
          className="rounded-[24px] sm:rounded-[36px] bg-white border border-slate-200/90 shadow-[0_4px_24px_rgba(0,0,0,0.03)] p-4 sm:p-8 lg:p-12 transition-all duration-300"
        >
            ...
            >
              ...
              <div className="col-span-12 lg:col-span-6 flex flex-col justify-center">
                ...
                <div className="mb-2 sm:mb-3">
                  <span className="text-[11px] sm:text-xs font-medium text-slate-400">PRD §02 Architecture</span>
                </div>

                {/* Title */}
                <h3 className="text-xl sm:text-2xl lg:text-[34px] font-normal text-neutral-900 tracking-[-0.025em] leading-[1.2]">
                  {activeTab.title}
                </h3>

                {/* Description */}
                <p className="mt-3 sm:mt-4 text-xs sm:text-sm lg:text-base text-neutral-600 font-normal leading-relaxed">
                  {activeTab.description}
                </p>

                {/* Specification Grid (Clean 2x2 micro-cards on tablet/desktop, adaptive on mobile) */}
                <div className="mt-5 pt-4 sm:pt-5 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3">
                  {activeTab.specs.map((spec) => (
                    <div key={spec.label} className="p-2.5 rounded-xl bg-slate-50/80 border border-slate-100/90 flex flex-col">
                ...
                <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                  ...
                  <Link
                    ...
                    className="w-full sm:w-auto px-5 py-3 rounded-full bg-white hover:bg-blue-50/50 text-neutral-700 hover:text-[#0364FF] border border-slate-200/90 hover:border-blue-200 text-xs sm:text-sm font-semibold tracking-tight transition-all flex items-center justify-center gap-1.5 group"
                  >
                    <span>{activeTab.ctaSecondary.label}</span>
                    <ArrowUpRight className="w-3.5 h-3.5 text-neutral-400 group-hover:text-[#0364FF] group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
                  </Link>
                </div>
              </div>

              {/* ─── Right Column: High-Fidelity UI Showcase Card (hidden on mobile, visible on desktop lg+) ─── */}
              <div className="hidden lg:block lg:col-span-6">
                <div className="rounded-2xl sm:rounded-3xl card-blue-pop card-specular-rim overflow-hidden">
                  {/* Browser / Console Chrome Bar */}
                  <div className="px-3.5 sm:px-4 py-2.5 sm:py-3 bg-white border-b border-slate-200/80 flex items-center justify-between gap-2">
                    ...
                    <div className="px-2.5 py-1 rounded-md bg-slate-50 border border-slate-200 text-[10px] font-mono text-neutral-500 truncate max-w-[140px] sm:max-w-[240px]">
                      https://{activeTab.mockup.url}
                    </div>

                    <div className="flex items-center gap-1 text-[9px] font-bold text-neutral-600 bg-slate-100 px-2 py-0.5 rounded shrink-0">
                      <span>SCHEMA ACTIVE</span>
                    </div>
                  </div>

                  {/* Console Interior Workspace */}
                  <div className="p-3.5 sm:p-5 lg:p-6 space-y-3 sm:space-y-4 font-sans select-none bg-[#fbfbfd]">
                    {/* Card 1: Active Programme Header */}
                    <div className="p-3.5 sm:p-4 rounded-xl sm:rounded-2xl bg-white border border-blue-100/70 shadow-[0_4px_16px_rgba(3,100,255,0.06),inset_0_1px_0_rgba(255,255,255,0.9)] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5 sm:gap-3">
                      <div className="flex items-center gap-2.5 sm:gap-3">
                        <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-blue-50/80 border border-blue-100 text-[#0364FF] flex items-center justify-center shrink-0">
                      ...
                      <span className="text-[9px] font-mono font-semibold text-neutral-800 bg-slate-100 px-2 py-1 rounded-md border border-slate-200 shrink-0 self-start sm:self-auto">
                        {activeTab.mockup.modelType}
                      </span>
                    </div>

                    {/* Card 2: Connected Platform Adapters (Adaptive 1-col on phone, 3-col on desktop) */}
                    <div className="p-3 sm:p-3.5 rounded-xl sm:rounded-2xl bg-white border border-blue-100/70 shadow-[0_4px_16px_rgba(3,100,255,0.06),inset_0_1px_0_rgba(255,255,255,0.9)] flex flex-col gap-2">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5 sm:gap-2 pt-1">
                        {activeTab.mockup.platforms.map((platform) => (
                          <div
                            key={platform}
                            className="p-2 rounded-xl bg-slate-50 border border-slate-200/60 flex items-center gap-1.5 text-[10px] sm:text-[9px] font-medium text-neutral-700 truncate"
                          >
                    ...
                    <div className="p-3 sm:p-3.5 rounded-xl sm:rounded-2xl bg-white border border-slate-200/80 shadow-[0_2px_8px_rgba(0,0,0,0.02)] flex flex-col gap-2">
                      ...
                      <div className="p-2 sm:p-2.5 rounded-xl bg-slate-50 border border-slate-100 text-[10px] font-mono text-neutral-600 truncate">
                        {activeTab.mockup.trackingDomain}
                      </div>
                    </div>

                    {/* Card 4: Double-Entry Ledger Stamp (PRD §15) */}
                    <div className="p-3 sm:p-3.5 rounded-xl sm:rounded-2xl bg-neutral-900 text-white flex flex-col gap-1.5 shadow-sm overflow-hidden">
                      <div className="flex items-center justify-between text-[9px] font-mono text-slate-400">
                        <span>DOUBLE-ENTRY USD LEDGER (MNY-01)</span>
                        <span className="text-emerald-400 font-semibold">BALANCED</span>
                      </div>

                      <div className="space-y-0.5 text-[9px] sm:text-[10px] font-mono text-slate-200 overflow-x-auto no-scrollbar">
                        <div className="truncate">{activeTab.mockup.ledgerEntry.dr}</div>
